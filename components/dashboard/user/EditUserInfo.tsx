...
const EditUserInfo: React.FC<EditUserInfoProps> = ({ user }) => {
  ...
  const onSubmit = async (data: Record<string, unknown>) => {
    try {
      setLoading(true);
      const trimmedData = removeFalsy(data as EditUserFormFields);
      const token =
        (session as { access_token?: string } | null)?.access_token ?? "";
      const payload: Record<string, unknown> = { ...trimmedData };
      if (data.role !== undefined) payload.role = data.role;
      const result = await updateUser(
        user.id,
        payload as Partial<EditUserFormFields> & { role?: string },
        token
      );
      if (result.data) {
