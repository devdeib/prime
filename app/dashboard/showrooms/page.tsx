...
export default function DashboardShowroomsPage() {
  ...
  const submitCreate = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/be/showrooms", {
        method: "POST",
        credentials: "include",
  ...
  const remove = async (id: number) => {
    if (!confirm(t("dashboard.deleteShowroomConfirm"))) return;
    const res = await fetch(`/api/be/showrooms/${id}`, {
      method: "DELETE",
      credentials: "include",
    });
    if (!res.ok) {
    ...
    <div className={styles.page}>
      <div className={styles.hero}>
        <div>
          ...
          <h1 className={styles.title}>{t("dashboard.showroomsMenu")}</h1>
          <p className={styles.subtle}>Manage showroom spaces, gallery images, and presentation order in a cleaner workspace.</p>
        </div>
      </div>
      ...
      <Card className={`${styles.panel} mb-4`}>
        <Card.Body>
          <Card.Title className={styles.panelTitle}>
            {editingId == null
              ? t("dashboard.addShowroom")
              : `${t("dashboard.editShowroom")} #${editingId}`}
          </Card.Title>
          <Form
